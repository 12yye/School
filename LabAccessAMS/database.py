...
# Retrieve access logs for a specific student by student ID
def get_access_logs_for_student(student_id):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT student_id, sign_in_time, sign_out_time FROM access_logs WHERE student_id = ?", (student_id,))
        return cursor.fetchall()
...
def search_by_date_range(start_date, end_date):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        query = """
        SELECT student_id, sign_in_time, sign_out_time 
        FROM access_logs 
        WHERE date(sign_in_time) BETWEEN ? AND ?
        """
        cursor.execute(query, (start_date, end_date))
        return cursor.fetchall()

# Search logs by specific time range within a date
def search_by_time_range(start_datetime, end_datetime):
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        query = """
        SELECT student_id, sign_in_time, sign_out_time 
        FROM access_logs 
        WHERE sign_in_time BETWEEN ? AND ?
        """
